...
// Hanya bisa edit jika status salah satu INVALID
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($file['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($ext, $allowed)) {
            ...
            if (!move_uploaded_file($file['tmp_name'], $uploadPath)) {
                ...
                // Hapus file lama
                $oldPath = "../../upload/" . $data['file_bukti'];
    ...
    if (empty($errors)) {
        // Reset status verifikasi setelah edit
        $stmt = $conn->prepare("UPDATE pengajuan_skkm SET nama_kegiatan = ?, file_bukti = ?, status_verifikasi_bem = 'Pending', status_verifikasi_kemahasiswaan = 'Pending', catatan_bem = '', catatan_kemahasiswaan = '', tanggal_verifikasi_bem = NULL, tanggal_verifikasi_kemahasiswaan = NULL WHERE id_pengajuan = ?");
        $stmt->bind_param("ssi", $nama_kegiatan, $newFileName, $id_pengajuan);
        if ($stmt->execute()) {
            $success = true;
        } else {
...
<div class="container mt-5">
    ...
    <form method="POST" enctype="multipart/form-data" class="card p-4 shadow">
        ...
        <div class="mb-3">
            ...
            <small class="text-muted">Biarkan kosong jika tidak ingin mengganti file bukti.</small>
            <br>
