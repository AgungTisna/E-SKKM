...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result->num_rows > 0) {
        ...
        $cek_duplikat = $conn->prepare("SELECT id_user FROM user WHERE username = ? AND id_user != ?");
        $cek_duplikat->bind_param("si", $username_baru, $id_user);
        $cek_duplikat->execute();
        $res_duplikat = $cek_duplikat->get_result();

        if ($res_duplikat->num_rows > 0) {
            $notif = "<div class='alert alert-warning'>Username baru sudah digunakan!</div>";
        } else {
...
<head>
    ...
    <title>Ubah Username Kemahasiswaan</title>
...
<div class="profile-container">
    <div class="profile-sidebar">
        ...
        <br><br>
        <a href="profile_kemahasiswaan.php"><button class="btn btn-outline-dark">Kembali</button></a>
    </div>
