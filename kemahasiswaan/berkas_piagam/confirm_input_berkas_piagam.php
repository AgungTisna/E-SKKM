...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['simpan']) && isset($_POST['data'])) {
    ...
    foreach ($dataRows as $row) {
        list($id_ormawa, $nama_kegiatan, $nama_penerima, $tanggal_kegiatan, $tanggal_pengajuan, $nomor_sertifikat, $keterangan) = explode('|', $row);
        ...
        $query = "INSERT INTO berkas_piagam 
            (nama_kegiatan, id_ormawa, id_kemahasiswaan, nama_penerima, tanggal_kegiatan, tanggal_pengajuan, tanggal_dikeluarkan, nomor_sertifikat_piagam, keterangan)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);

        if (!$stmt) {
            die("Prepare gagal: " . $conn->error . "<br>Query: $query");
        }
        ...
        $stmt->bind_param(
            ...
            $tanggal_dikeluarkan,
            $nomor_sertifikat,
            $keterangan // jaga panjang keterangan
        );
    ...
    <div class="container mt-5">
        <div class="alert alert-success">
            <strong>{$berhasil}</strong> data berhasil disimpan ke Berkas Piagam.
        </div>
        <a href="input_berkas_piagam.php" class="btn btn-primary">← Upload Lagi</a>
        <a href="detail_berkas_piagam.php" class="btn btn-secondary ms-2">Lihat Data</a>
    </div>
    HTML;
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
    ...
    <head>
        <meta charset="UTF-8">
        <title>Konfirmasi Berkas Piagam</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
        <style>
    ...
    <body>
    <div class="container mt-5">
        <h3 class="mb-4">Konfirmasi Data Piagam</h3>
        <form method="post">
...
HTML;
    ...
    echo <<<HTML
        ...
        <div class="mt-3">
            ...
            <a href="input_berkas_piagam.php" class="btn btn-secondary ms-2">← Kembali</a>
        </div>
        </form>
    </div>
    </body>
    ...
    echo "<script>alert('File tidak valid atau tidak ditemukan.'); window.location.href='input_berkas_piagam.php';</script>";
}
?>
