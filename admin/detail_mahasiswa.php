...
$totalQuery = "SELECT COUNT(*) AS total FROM user WHERE role = 'Mahasiswa'";
$totalResult = $conn->query($totalQuery);
$totalRow = $totalResult->fetch_assoc();
$totalMahasiswa = $totalRow['total'];
$totalPages = ceil($totalMahasiswa / $limit);

// Query untuk mengambil data mahasiswa dengan pagination
$query = "SELECT u.id_user, u.nama, u.email, u.username, u.password, d.nim, d.prodi, d.angkatan 
          FROM user u 
          INNER JOIN user_detail_mahasiswa d ON u.id_user = d.id_user 
...
<div class="container mt-3 d-flex justify-content-between">
    ...
    <h3 class="text-center flex-grow-1">Detail Mahasiswa</h3>
    <a href="tambah_mahasiswa.php"><button class="btn-custom">Tambah Mahasiswa</button></a>
</div>
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<tr>
                    ...
                    <td>{$row['password']}</td>
                    <td>
                        <a href='hapus_mahasiswa.php?id={$row['id_user']}' class='btn btn-danger' onclick='return confirm(\"Apakah Anda yakin ingin menghapus?\")'>Hapus</a>
                    </td>
                  </tr>";
            $no++;
        }
...
<div class="container text-center mt-3">
    <p>Menampilkan <?php echo min($limit, $totalMahasiswa - $start); ?> dari <?php echo $totalMahasiswa; ?> mahasiswa</p>
    <nav>
