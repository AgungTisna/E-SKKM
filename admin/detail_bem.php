...
// Query untuk menghitung total data BEM
$totalQuery = "SELECT COUNT(*) AS total FROM user WHERE role = 'BEM'";
$totalResult = $conn->query($totalQuery);
$totalRow = $totalResult->fetch_assoc();
$totalBEM = $totalRow['total'];
$totalPages = ceil($totalBEM / $limit);

// Query untuk mengambil data BEM dengan pagination
$query = "SELECT u.id_user, u.nama, u.email, u.username, u.password, d.jabatan 
          FROM user u 
          INNER JOIN user_detail_bem d ON u.id_user = d.id_user 
          WHERE u.role = 'BEM'
          LIMIT $start, $limit";
...
<div class="container mt-3 d-flex justify-content-between">
    ...
    <h3 class="text-center flex-grow-1">Detail BEM</h3>
    <a href="tambah_bem.php"><button class="btn-custom">Tambah BEM</button></a>
</div>
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<tr>
                    ...
                    <td>{$row['password']}</td>
                    <td>
                        <a href='hapus_bem.php?id={$row['id_user']}' class='btn btn-danger' onclick='return confirm(\"Apakah Anda yakin ingin menghapus?\")'>Hapus</a>
                    </td>
                  </tr>";
            $no++;
        }
...
<div class="container text-center mt-3">
    <p>Menampilkan <?php echo min($limit, $totalBEM - $start); ?> dari <?php echo $totalBEM; ?> data BEM</p>
    <nav>
